...
const REGISTRY: SkillDefinition[] = [
	...
	},
	{
		...
		catalogDoc: [
			...
			'    通用 API 查询参数使用 args ["--params","{\\"key\\":\\"value\\"}"]；连接器也会兼容 path 中的 ?key=value 和旧的 --query 写法。',
			'    给自己发消息（优先使用高级命令，不要拼 /im/v1/messages 通用 API）: 先 args ["contact","+get-user","--as","user","--format","json"] 取 stdout.data.user.open_id；再 args ["im","+messages-send","--as","user","--user-id","<open_id>","--text","消息内容"]',
			'    给自己发本地文件附件: 优先 args ["drive","+upload","--as","user","--file","./相对路径.pdf","--name","文件名.pdf"] 取 stdout.data.url；再 im +messages-send --text 带上该链接。注意：im --file 需 im:resource 授权、路径必须相对当前目录、且不能与 --text 同用。',
			'    创建飞书日历日程: args ["calendar","+create","--as","user","--summary","标题","--start","2026-07-18T15:00:00+08:00","--end","2026-07-18T16:00:00+08:00","--description","可选说明","--format","json"]',
